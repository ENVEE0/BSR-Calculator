/*
 * scripts.js
 * BSR Calculator - main UI logic and calculations
 * - Contains data tables, settings, calculation routines, and UI handlers
 * - Keep changes minimal and well-tested; this file is loaded on every page
 */
...
// Data constants (farming rewards, exp tables, default settings) moved to `data.js`.
// Ensure `data.js` is loaded before this script so variables like `farmingRewardsByLevel`,
// `expData`, and `defaultSettings` are available globally.
...
// Build quick lookup tables for kan/ascension costs using settings so older
// functions that reference `kanAscData` / `kanExpData` remain compatible.
const kanAscData = {};
const kanExpData = {};
{
    const milestones = [1, 20, 30, 40, 50, 60, 70, 80, 90, 100];
    milestones.forEach(base => {
        kanAscData[base] = {};
        kanExpData[base] = {};
        milestones.forEach(next => {
            if (next <= base) return;
            const key = `${base}-${next}`;
            kanAscData[base][next] = settings.character?.ascensionKans?.[key] ?? 0;
            kanExpData[base][next] = settings.character?.kans?.[key] ?? 0;
        });
    });
}
...
// Wheel/select handlers are implemented in `utils.js` (attachWheelAndSelectHandlers).
...
document.addEventListener('DOMContentLoaded', () => {
    // Attach wheel/select handlers after DOM ready
    if (typeof attachWheelAndSelectHandlers === 'function') attachWheelAndSelectHandlers();
